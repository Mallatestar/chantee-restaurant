...
public class OrderFinishedCommandTest {
    ...
    @InjectMocks
    public OrderFinishedCommand testCommand;
    ...
    public void setUp() {
        ...
        when(session.getAttribute("delivery")).thenReturn(new LinkedList<>());
    }

    @Test
    public void execute1() throws DAOException {
        doNothing().when(dao).changeOrderStage(1, "finished");
        assertEquals(testCommand.execute(request, response), "/manager-panel");
    }

    @Test
    public void execute2() throws DAOException {
        doThrow(new DAOException()).when(dao).changeOrderStage(1, "finished");
